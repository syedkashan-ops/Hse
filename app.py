...
st.set_page_config(page_title="HSE ProGen AI Assistant V9", page_icon="🦺", layout="wide", initial_sidebar_state="collapsed")
...
"""
    ...
    # Streamlit widget state can retain the old empty values from the first
    # page render. Push the newly generated values into each text-area key
    # before those widgets are created on this rerun. Without this, the AI
    # response exists in session_state.data but the visible fields can remain
    # blank.
    for section_key, section_values in st.session_state.data.items():
        for field_key, field_value in section_values.items():
            st.session_state[f"{section_key}_{field_key}"] = field_value
...
st.title("🦺 HSE ProGen AI Assistant — V9")
st.caption("Streamlit + GitHub | Mobile Chrome friendly | No extension | Gemini AI | Human review required | Visible-output-fixed V9")
...
with tabs[6]:
    ...
    st.download_button("Download JSON", data_json, "progen_v9_review.json", "application/json")
    text_parts=[]
    for sec,content in st.session_state.data.items():
        ...
        for k,v in content.items():
            text_parts += [k, str(v), ""]
    data_txt="\n".join(text_parts)
    st.download_button("Download TXT", data_txt, "progen_v9_review.txt", "text/plain")
