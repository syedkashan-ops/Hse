...
st.set_page_config(page_title="HSE ProGen AI Assistant V6", page_icon="🦺", layout="wide", initial_sidebar_state="collapsed")
...
"""
    # Build a real JSON Schema from the field template. The template itself is
    # sample data, not a valid structured-output schema. Passing the sample
    # dictionary directly can trigger Pydantic errors such as
    # "Extra inputs are not permitted" in the google-genai SDK.
    def to_json_schema(value):
        if isinstance(value, dict):
            return {
                "type": "object",
                "properties": {k: to_json_schema(v) for k, v in value.items()},
    ...
    # Defensive normalization: keep every expected field and make every value
    # a string so the UI/copy mode cannot break on null/list/object values.
    for section, fields in schema.items():
        if not isinstance(parsed.get(section), dict):
            parsed[section] = {}
        for field in fields:
            value = parsed[section].get(field, "")
            if value is None:
                value = ""
...
st.title("🦺 HSE ProGen AI Assistant — V6")
st.caption("Streamlit + GitHub | Mobile Chrome friendly | No extension | Gemini AI | Human review required")
...
with tabs[6]:
    ...
    st.download_button("Download JSON", data_json, "progen_v6_review.json", "application/json")
    text_parts=[]
    for sec,content in st.session_state.data.items():
        ...
        for k,v in content.items():
            text_parts += [k, str(v), ""]
    data_txt="\n".join(text_parts)
    st.download_button("Download TXT", data_txt, "progen_v6_review.txt", "text/plain")
